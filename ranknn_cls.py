...
def ranknn_cls_seconde_process(data_file,label_encoders=None,imputers=None,scalers=None):
...
class CatEmbeddings_cls_ranknn(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        super(CatEmbeddings_cls_ranknn, self).__init__()
...
class RankNN_cls(nn.Module):
    ...
    def __init__(
        ...
        """
        super(RankNN_cls, self).__init__()
        self.embeddings = CatEmbeddings_cls_ranknn(projection_dim, categorical_cardinality, embedding_dim)
...
class RankLitNN_cls(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        """
        super(RankLitNN_cls, self).__init__()
        self.save_hyperparameters()

        # Creates an instance of the NN model defined above
        self.model = RankNN_cls(
...
def train_ranknn_cls(continuous_dim, train_set, eval_set, categorical_cardinality,fea_cat,fea_value, hparams=None):
    """
...
def ranknn_cls_train(seed,fold_n):
    data,label_encoders,imputers,scalers = ranknn_cls_seconde_process(args.data_process_dir+'data_train.pkl')
    encoders = {'label_encoders':label_encoders,'imputers':imputers,'scalers':scalers}
    with open(args.encoder_info_dir+'encoders_ranknn_cls.pkl','wb') as f:
    ...
    for i,(train_index,eval_index) in enumerate(skf.split(data['X'],y_combine)):
        ...
        # traing
        model = train_ranknn_cls(len(data['feature_value']), train_set, eval_set, categorical_cardinality,data['feature_cat'],data['feature_value'])
        model.eval()

        # fix the result shift between different fold
        rate_postive=sum(y_train_cls)/len(train_index)
        y_hat_raw = model(train_set.tensors[0],train_set.tensors[1],).cpu().detach().numpy()
        ...
        torch.save(model_info,args.model_dir+'ranknn_cls_fold%s.pth'%(i))
        with open(args.model_dir+'ranknn_cls_fold%s.pkl'%(i),'wb') as f:
